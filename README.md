...
### Переменные
Все переменные начинаются с префикса `n` *(от англ. **name** — «погоняло»)* и числовой идентификатор.<br>
...
Все аргументы функции именуются с префикса `n` *(от азерб. **nəticə** — «заключение»)* и порядкового номера, необязательные аргументы именуются как обычные переменные.
...
> * Дочерний класс — `NXXXXXXXXXXnYYYYYYYYYY`
...
* Внутри группы импорты сортируются по возрастанию.
